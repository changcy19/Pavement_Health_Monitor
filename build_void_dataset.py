...
CASE_PATTERN = re.compile(
    ...
    r"(?P<void>[OABC])_V(?P<v>\d+(?:\.\d+)?)_M(?P<m>\d+(?:\.\d+)?)$",
    re.IGNORECASE,
)
...
def parse_case_name(path: Path) -> CaseLabel:
    ...
    void_code = match.group("void").upper()
    ...
    return CaseLabel(
        ...
        void_code=void_code,
        void_width_cm=float(VOID_MAP.get(void_code, np.nan)),
...
def extract_features(csv_path: Path, label: CaseLabel, point: str) -> dict:
    ...
    stress = (
        pd.to_numeric(frame.loc[mask, "stress"], errors="coerce").to_numpy(dtype=float)
        if "stress" in frame.columns
        else np.array([], dtype=float)
    )

    dt = float(np.median(np.diff(time))) if len(time) > 1 else 1.0
    accel_c = accel - float(np.mean(accel))
    strain_c = strain - float(np.mean(strain))
...
def extract_point_features(csv_path: Path, label: CaseLabel, point: str) -> dict:
    feat = extract_features(csv_path, label, point)
    feat.pop("file", None)
    feat.pop("point", None)
    return feat
...
def build_dataset(data_dir: Path, csv_dir: Path, points: list[str]) -> pd.DataFrame:
    ...
    for odb_path in sorted(data_dir.glob("*.odb")):
        ...
        row = {**{k: v for k, v in asdict(label).items() if k not in {"stem", "void_code"}}}
        row["source_stem"] = label.stem
        row["void_width_cm"] = float(label.void_width_cm)
        ok = True
        for point in points:
            stem = odb_path.stem
            csv_path = csv_dir / f"{stem}__{point}.csv"
            if not csv_path.exists():
                ok = False
                missing.append(f"{odb_path.name}:{point}")
                break
            point_feat = extract_point_features(csv_path, label, point)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a feature table from exported CSV signals.")
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "dataset_void.csv", help="Output dataset CSV")
    parser.add_argument("--meta", type=Path, default=ROOT / "dataset_void_meta.json", help="Output metadata JSON")
    ...
    meta = {
        ...
        "points": args.points,
        "void_label_mapping": {k: v for k, v in VOID_MAP.items()},
    }
