...
def build_xy(df: pd.DataFrame):
    ...
    if missing:
        raise SystemExit(f"Missing columns in dataset_void.csv: {missing[:10]}")

    if "void_width_cm" not in df.columns:
        raise SystemExit("dataset_void.csv must contain void_width_cm as target column")

    groups = df["source_stem"] if "source_stem" in df.columns else df["file"]
    X = df[FEATURE_COLS].copy()
    X = X.replace([np.inf, -np.inf], np.nan).fillna(X.median(numeric_only=True))
    y = df["void_width_cm"].astype(float)
    return X, y, groups
...
def run_train(dataset_path: Path, save_dir: Path, test_size: float = 0.2, seed: int = 42):
    ...
    if df.empty:
        raise SystemExit("dataset_void.csv is empty")
    ...
    joblib.dump(pipe, save_dir / "void_regressor.joblib")

    test_pred = test_df[["source_stem", "file", "void_width_cm"]].copy()
    test_pred["pred_void_width_cm"] = pred
    test_pred["abs_error"] = (test_pred["pred_void_width_cm"] - test_pred["void_width_cm"]).abs()
    test_pred.to_csv(save_dir / "test_predictions.csv", index=False, encoding="utf-8-sig")

    case_summary = (
        test_pred.groupby("source_stem")
        .agg(
            true_void_width_cm=("void_width_cm", "first"),
            pred_void_width_cm=("pred_void_width_cm", mode_or_first),
            n_rows=("pred_void_width_cm", "size"),
            mae=("abs_error", "mean"),
        )
        .reset_index()
    )
...
def main() -> None:
    ...
    parser.add_argument("--dataset", type=Path, default=ROOT / "dataset_void.csv")
    parser.add_argument("--save", type=Path, default=ROOT / "model_out")
